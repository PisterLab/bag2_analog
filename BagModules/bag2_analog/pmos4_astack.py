...
class bag2_analog__pmos4_astack(Module):
    """Module for library bag2_analog cell pmos4_astack.
    ...
    def design(self, **params) -> None:
        ...
        if stack > 1:
            for i in range(stack):
                ...
                seg = seg_list[i]
                self.instances['XP'][i].design(w=w, l=lch, nf=seg, intent=intent)
        else:
            w = w_list[0]
            lch = lch_list[0]
